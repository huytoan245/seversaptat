...
new = "'EnsureListSelectionVisible','PreviewCacheKey','SchedulePreviewPrefetch','PreviewCacheCapacity','TryGetCachedPreview','SemaphoreSlim _previewRenderGate','ConfigureRightPaneRatio(inner, 0.14, 215, 260)','cutPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100))'"
if old not in s:
...
new = "(Join-Path $Project 'patch_v244.py'), (Join-Path $Project 'prepare_build_v244.py'), (Join-Path $Project 'patch_v245.py'), (Join-Path $Project 'patch_v245b.py'), (Join-Path $Project 'patch_v245c.py'), (Join-Path $Project 'patch_v245d.py'), (Join-Path $Project 'prepare_build_v245.py'), (Join-Path $Project 'regression_test.py'),"
if old not in s:
