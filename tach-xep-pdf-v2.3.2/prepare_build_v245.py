...
# Apply v2.4.5 after v2.4.4 source has been reconstructed.
old = "& python (Join-Path $Project 'patch_v244.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v244.py failed' } }"
new = "& python (Join-Path $Project 'patch_v244.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v244.py failed' }; & python (Join-Path $Project 'patch_v245.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v245.py failed' } }"
if old not in s:
...
new = "'EnsureListSelectionVisible','PreviewCacheKey','SchedulePreviewPrefetch','PreviewCacheCapacity','TryGetCachedPreview','SemaphoreSlim _previewRenderGate'"
if old not in s:
...
new = "(Join-Path $Project 'patch_v244.py'), (Join-Path $Project 'prepare_build_v244.py'), (Join-Path $Project 'patch_v245.py'), (Join-Path $Project 'prepare_build_v245.py'), (Join-Path $Project 'regression_test.py'),"
if old not in s:
