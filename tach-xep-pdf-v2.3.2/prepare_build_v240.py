...
new="& python (Join-Path $Project 'patch_v235g.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v235g.py failed' }; & python (Join-Path $Project 'patch_v240.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v240.py failed' }; & python (Join-Path $Project 'patch_v240b.py'); if ($LASTEXITCODE -ne 0) { throw 'patch_v240b.py failed' } }"
if old not in s: raise SystemExit('v235g invocation marker missing')
s=s.replace(old,new,1)

old="'ConfigureRightPaneRatio')"
new="'ConfigureRightPaneRatio','Khôi phục PDF gốc','RESTORE_ORIGINAL_FAIL','Panel2Collapsed = true','125%','150%','200%','SetZoomPercent','PDF preview aspect ratio mismatch','.restore.rollback')"
...
new="(Join-Path $Project 'prepare_build_v235f.py'), (Join-Path $Project 'patch_v240.py'), (Join-Path $Project 'patch_v240b.py'), (Join-Path $Project 'prepare_build_v240.py'), (Join-Path $Project 'regression_test.py'),"
