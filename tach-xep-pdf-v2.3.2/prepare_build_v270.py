...
new_snapshot = "(Join-Path $Project 'patch_v260.py'), (Join-Path $Project 'prepare_build_v260.py'), (Join-Path $Project 'patch_v261.py'), (Join-Path $Project 'patch_v261_identity.py'), (Join-Path $Project 'patch_v261_scroll_extent.py'), (Join-Path $Project 'prepare_build_v261.py'), (Join-Path $Project 'patch_v270_model.py'), (Join-Path $Project 'patch_v270_ui.py'), (Join-Path $Project 'patch_v270_compact_top.py'), (Join-Path $Project 'patch_v270_list.py'), (Join-Path $Project 'patch_v270_merge_extract.py'), (Join-Path $Project 'patch_v270_merge_safety.py'), (Join-Path $Project 'patch_v270_merge_trace.py'), (Join-Path $Project 'patch_v270_merge_finalize.py'), (Join-Path $Project 'patch_v270_fullpage_render.py'), (Join-Path $Project 'patch_v270_tests.py'), (Join-Path $Project 'patch_v270_preview_smoke.py'), (Join-Path $Project 'patch_v270_restore_safe.py'), (Join-Path $Project 'patch_v270_launcher.py'), (Join-Path $Project 'prepare_build_v270.py'), (Join-Path $Project 'regression_test.py'),"
...
if marker in s:
    notes = ('\n- Mở PDF ở thư mục mới: tự dọn danh sách cũ khi tất cả tệp đã hoàn thành; nếu còn tệp chưa hoàn thành sẽ hỏi trước, không xóa PDF nguồn.'
        ...
        '\n- Trang ghép sau khi đã cắt luôn Preview/Xuất nguyên trang, không đi qua CropHalf.'
